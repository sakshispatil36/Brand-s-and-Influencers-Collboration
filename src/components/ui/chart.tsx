...
// Custom payload type to replace TooltipPayload
type MyPayloadItem = {
  name?: string;
  dataKey?: string;
  value?: string | number;
...
// Helper function for extracting config from payload
function getPayloadConfigFromPayload(config: ChartConfig, payload: MyPayloadItem, key: string) {
  return config[key] || { label: payload.name || key, icon: undefined, color: payload.color };
}

// Export all components safely
